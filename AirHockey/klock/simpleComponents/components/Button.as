...
package klock.simpleComponents.components
{
	...
	public class Button extends BaseElement
	{
		 ...
		 * @param label The string to use of this component.
		 * @param The parent DisplayObjectContainer.   */	
		...
		override protected function AddChilds():void
		{
			...
			addChild( _tf = new Label({x:BaseLayout.LEFT , y:0, _width: _width, _height:_height}, _label, this ))
		//	_tf.setBorder( true)
		...
		override protected function Listener( event:Event ):void
		{
			...
			if( mode != -1 ) {
				...
				BaseLayout.DRAW_OUTLINE(    this.graphics, _width, _height, ( _pushMode ) ? 0 : mode )
				BaseLayout.DRAW_FILLIN ( _fillin.graphics, _width, _height, ( _pushMode ) ? 2 : mode )
		...
		override public function Draw( ):void
		{
			...
			BaseLayout.DRAW_FILLIN ( _fillin.graphics, _width, _height,( _pushMode ) ? 2 : 0 )

			_tf.text = _label;
			_tf.setSize( _width -BaseLayout.LEFT*2, _tf.height )
			_tf.y = Math.round( _height / 2 - _tf.height / 2)-2;
			_tf.Draw()
		}
